...
dc.loadprogram = function(cname){
  ...
  $ajaxUtils.sendGetRequest(
    ...
    function(egitimjson){     
        ...
        for(var k=0;k<egitimjson[0].sinifno.length;k++){
          for(var a=0;a<egitimjson[0].sinifno[k].donem.length;a++){
            if(egitimjson[0].sinifno[k].donem[a].ad===cname){
            buildAndShowprogramilkHTML(egitimjson[0].sinifno[k].donem[a].ders);}
          }
        }
       
          for(var a=0;a<egitimjson[1].donem.length;a++){
            if(egitimjson[1].donem[a].ad===cname){
            buildAndShowprogramilkHTML(egitimjson[1].donem[a].ders);}
          }
...
finalHtml += "<div>";
    ...
    for (var k = 0; k < 6; k++) {
     ...
     var html=sınıfhtml;
     var a =egitimjson[k].x[i]; 
     html =
      insertProperty(html, "name", a);    
      finalHtml += html;
